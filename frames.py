...
class BaiLanFrames:  # 帧生成
    ...
    def scene_2(self):
        ...
        for i in range(len(fzs_left)):
            _pst = paste_image_return(fzs_right[i], fzs_left[i], 0, 0)
            fzs.append(_pst)
    ...
    def scene_6(self):
        ...
        # 鞭炮部分, 第一支 22.24 - 23.24
        w_msk = Image.open("./src/白遮罩.png")

        fzs = []

        im = Image.open("./src/鞭炮.png")
        fire = Image.open("./src/火花2.png")
        for i in range(10):
            bx = random.randint(0, 1920 - 500)
            by = random.randint(0, 1080 - 850)
            paste_image(pt, im, bx, by, 277, 765)
            fz = generate_turn_around_frames(w_msk, 0, 30, [[bx - 50, by + 766], [bx - 77, by + 31]],
                                             wh=(1060, 1060), color=(0, 0, 0, 0), bgimg=pt)
            for n in range(len(fz)):
                paste_image(fz[n], fire, bx + 54, by + 553, 253, 128)
            fzs.append(fz)
            self.clear_pt((255, 255, 255, 0))
        ...
        for n in range(191):  # 7帧一炮
            ...
            for f_index in flags:
                ...
                try:
                    pt = paste_image_return(pt, fzs[f_index][t_frame], 0, 0, fill=(255, 255, 255, 0))
                except IndexError:
                    pass
                except MemoryError:
                    pass
            ...
            pt.convert("RGB").save(f"./temp/scene_5/{n}.jpg", quality=90)
            # ret_frames.append(pt.copy())
            print(flags)
